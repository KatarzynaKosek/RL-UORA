...
class UORA():
    ...
    def backoff_procedure(self, n_ru, n_sta, alpha=1, first='false', counters={}, times_dict={}, thr_results={}, txop=False):
        transmission = 0
        collission = 0
        if first == 'false':
            counters = {}
        ...
        for b in range(0, ru_sta_array.shape[0]):
            ...
            if len(e[0]) == ru_sta_array.shape[1]:
                empty_ru.append(1)
                self.reward -= 1.5  # 1 #penalty for empty RUs
            else:
                empty_ru.append(0)
            ...
            if len(d[0]) == 1:
                ...
                self.reward += 3  # 4 #reward for successful RUs
            else:
            ...
            if len(d[0]) > 1:
                ...
                self.reward -= 2  # 3.5 #penalty for unsuccessful RUs
            else:
    ...
    def reset(self, n_sta, n_ru):
        counters, times_dict, thr_results, alpha = self.backoff_procedure(
            n_ru, n_sta, 1, 'false', counters={}, times_dict={}, thr_results={}, txop=False)
        state = [0]
        return state, counters, times_dict, thr_results, alpha

    def stepreset(self, n_sta, n_ru, alpha):
        counters, times_dict, thr_results, alpha = self.backoff_procedure(
            n_ru, n_sta, alpha, 'false', counters={}, times_dict={}, thr_results={}, txop=False)
        return counters, times_dict, thr_results, alpha

    def step(self, action, n_ru, n_sta, counters, times_dict, thr_results, alpha, time):
        self.reward = 0
        self.done = 0
        ...
        while time1 < time:
            counters, times_dict, thr_results, alpha = self.backoff_procedure(
                n_ru, n_sta,  alpha, 'true', counters, times_dict, thr_results)
            time1 += 0.1
