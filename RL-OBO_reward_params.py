...
from UORA_RL_reward_params import UORA
...
class DQN:
    ...
    def replay(self):
      ...
      #  print("epsilon:", self.epsilon, "stations:", n_sta)

        '''
        if self.epsilon<.01:
                print("------------------------------------")
                print("------------------------------------")
                print(self.epsilon, n_sta)
                print("------------------------------------")
                print("------------------------------------")
        '''
...
def train_dqn(episode, steps):  # DQN training
    ...
    for e in range(episode):
        ...
        state, counters, times_dict, thr_results, alpha = env.reset(
            n_sta, n_ru, reward_success, reward_collision, reward_empty)
        print("reward-succes, reward-empty, reward-collision:",
              reward_success, reward_empty, reward_collision)
            ...
            if i > 0:
                ...
                if (i % 500 == 0) & (i < 2001):
                    ...
                    counters, times_dict, thr_results, alpha = env.stepreset(
                        n_sta, n_ru, alpha, reward_success, reward_collision, reward_empty)
                    reward, next_state, done, counters, times_dict, thr_results, alpha = env.step(
                        action, n_ru, n_sta, reward_success, reward_collision, reward_empty, counters, times_dict, thr_results, round(alpha, 1), 1)
...
if __name__ == '__main__':
    ...
    f = open("RL-OBO-parameters-training.txt", 'a', encoding='UTF8', newline='', )
    writer = csv.writer(f, delimiter=',')
    f.close()
    ...
    writer = csv.writer(f, delimiter=',')
    writer.writerow(['no of stas', 'througput', 'no of RUs','p_col', 'efficiency', 'throughput-faifness', 'pc-fairness', 'avg_del','delay_min','delay_max']) 
    f.close()

    f = open("RL-OBO-parameters-testing.txt", 'a', encoding='UTF8', newline='', )
    writer = csv.writer(f, delimiter=',')
    f.close()
    ...
    ep = 15  # no of episodes for DQN training
    max_steps = 2000  # no of steps per episode
    reward = 0
    score = 0

    # TRAINING
    for reward_empty in np.arange(1, 3.5, 0.5):
        for reward_collision in np.arange(1, 3.5, 0.5):
            N_RU_array = [4]
            n_sta = 4

            reward_success = 3
            reward_collision = reward_collision
            reward_empty = reward_empty

            f = open("RL-OBO-results-training.csv", 'a', encoding='UTF8', newline='', )
            writer = csv.writer(f, delimiter=',')
            writer.writerow(
                ['reward_success', 'reward_collision', 'reward_empty'])
            writer.writerow([reward_success, reward_collision, reward_empty])
            f.close()
        ...
        '''
            ...
            for n_ru in N_RU_array:
                ...
                plt.title(str(reward_success) +
                          str(reward_collision)+str(reward_empty))
                plt.xlabel('steps')
                ...
                # writing the results
                f = open("RL-OBO-parameters-training.txt", 'a',
                         encoding='UTF8', newline='',)
                writer = csv.writer(f, delimiter=',')
...
'''
    ...
    for base in [5]: #base for dynamic selection of the number of stations present in the network
        ep_test=3 #no of episodes for DQN testing
        max_steps_test = 300 #maximum number of steps
        loss_test, thr_ep_test, col_ep_test, alpha_ep_test, efficiency_ep_test= test_dqn(ep_test, agent, base, max_steps_test)         
        
        #plotting
        plt.plot([i for i in range(len(efficiency_ep_test)-max_steps_test,len(efficiency_ep_test))], efficiency_ep_test[(len(efficiency_ep_test)-max_steps_test):len(efficiency_ep_test)])
        plt.xlabel('steps')
        plt.ylabel('last ep efficiency')
        plt.show()      
        
        plt.plot([i for i in range(len(efficiency_ep_test)-max_steps_test,len(efficiency_ep_test))], col_ep_test[(len(efficiency_ep_test)-max_steps_test):len(efficiency_ep_test)])
        plt.xlabel('steps')
        plt.ylabel('last ep pcol')
        plt.show()
        
        plt.plot([i for i in range(len(efficiency_ep_test)-max_steps_test,len(efficiency_ep_test))], alpha_ep_test[(len(efficiency_ep_test)-max_steps_test):len(efficiency_ep_test)])
        plt.xlabel('steps')
        plt.ylabel('last ep alpha')
        plt.show()
        ...
        plt.plot([i for i in range(len(efficiency_ep_test))], efficiency_ep_test)
        plt.xlabel('steps')
        plt.ylabel('efficiency')
        plt.show()
        ...
        f = open("RL-OBO-parameters-test.txt", 'a', encoding='UTF8', newline='',)
        writer = csv.writer(f, delimiter=',')
