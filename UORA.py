#
# Authors: K. Domino, K. Kosek-Szott
...
class UORA:
    ...
    def backoff_procedure(self, n_ru, n_sta, first='false', counters={}, times_dict={}, thr_results={}, txop=False):
        transmission = 0
        collission = 0
        if first == 'false':
            counters = {}
            times_dict = {}
            thr_results = {}
            results_all = [0]  # for gathering detailed statistics
            tx_time = 0
            time_spent = 0
            time_wait = 0
            h = 0  # contention counter
            c = 0
            le = 0  # detailed data recording interval (results_all)
            alpha = 1
            empty_ru = []
            successfull_ru = []
            unsuccessfull_ru = []
            send_frames_counter = np.zeros(int(n_sta))
            transmitted = np.zeros(int(n_sta))
            ...
            transmitted = counters['transmitted'][:int(n_sta)]
            h = counters['h']
            le = counters['le']
            c = counters['c']
            ...
            if n_sta > send_frames_counter.size:
                ...
                transmitted = np.append(
                    transmitted, np.zeros(n_sta-transmitted.size))
        ...
        for b in range(0, ru_sta_array.shape[0]):
            ...
            if len(e[0]) == ru_sta_array.shape[1]:  # check if there are any empty RUs
                empty_ru.append(1)
            else:
                empty_ru.append(0)

            if len(d[0]) == 1:  # check if there are any successful RUs
                successfull_ru.append(1)
            else:
                successfull_ru.append(0)

            if len(d[0]) > 1:  # check if there are any collisions/unsuccessful RUs
                unsuccessfull_ru.append(1)
            else:
                unsuccessfull_ru.append(0)

            if len(d[0]) == 1:  # successful RU
                ...
                if not txop:
                    for sta in d[0]:
                        ...
                        delay_cnt[sta] = 0
                    for sta in e[0]:
                        delay_cnt[sta] = delay_cnt[sta] + \
                            (self.times.transmission_time())
                else:
                    for sta in d[0]:
            ...
            elif len(d[0]) > 1:  # unsuccessful RUs
                for sta in d[0]:
                    transmitted[d] = False
        ...
        no = 10  # number of contention rounds after which data is gathered

        h = h+1  # contention cunter

        le += 1

        for sta in range(0, n_sta):
            if (h >= no):
                j = (sum(unsuccessfull_ru[-(no*n_ru):])/(sum(successfull_ru[-(
                    no*n_ru):]+unsuccessfull_ru[-(no*n_ru):]+empty_ru[-(no*n_ru):])))
                g = (sum(empty_ru[-(no*n_ru):])/(sum(successfull_ru[-(no*n_ru):]+unsuccessfull_ru[-(no*n_ru):]+empty_ru[-(no*n_ru):])))
                if ((j >= 0.33) & (g < 0.33)):
                    ocwmin[sta] = self.ocwmin
                    ocwm[sta] = self.ocwmax
                    alpha -= 0.1
                    alpha = max(0.1, alpha)
        ...
        counters['h'] = h
        counters['c'] = c
        counters['alpha'] = alpha
        counters['le'] = le

        if ~np.isnan(any(collision_cnt)) == True:
            colision_probability = collision_cnt / \
                (collision_cnt + send_frames_counter)
        else:
            colision_probability = 0
        ...
        if (le == 99):
            results_all.extend(throughput)

        thr_results['thr'] = throughput
        thr_results['colision_prob'] = colision_probability
        thr_results['success'] = success_probability

        return counters, times_dict, thr_results, transmitted, alpha
